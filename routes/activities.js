...
router.post("/getByLandmark", async (req, res) => {
    try {
...
router.use((req, res, next) => {
    if (!req.user.admin && !req.user.superAdmin) return res.json({ msg: "NOT ADMIN" }) //check if not admin or superadmin
    else next()
...
router.post("/deleteByTypeAndLandmark", async (req, res) => {
    try {
        const activity = await Activities.findOne({ type: req.body.name })
        if (!activity) return res.json({ msg: "ACTIVITY NOT FOUND" })
